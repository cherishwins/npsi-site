...
- `/disclosure/` is linked from the **footer colophon sitewide** (GITHUB · LINKEDIN · DISCLOSURE · COLOPHON) and from `/about/` — **never from the four-link nav**.
...
The imprint's **monthly periodical** — "A monthly account of the Canada–Korea relationship." Document IDs follow `NPSI-PL-NNN`; №01 (premier issue) closed 28 July 2026, integrated 29 July 2026. **The Ledger is the one NPSI product line with a stated cadence** (monthly; "№02 closes late August") — this is a deliberate, editor-directed carve-out from the "not a content stream" rule, which continues to govern the working papers and briefings. The Ledger does not join the four-link nav; it is reached from the home page's "The Pacific Ledger" section and direct URL. Page-scoped `pl-`-prefixed components (masthead with doublerule, stat band, timeline, energy charts as inline SVG on site tokens, the inverted "people card," memorial band, ledger/watch rows). Key facts that should remain consistent:
